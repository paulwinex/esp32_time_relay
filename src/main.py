...
class ControllerTitle(Controller):
    ...
    def get_title(self):
        ...
        return (str(self.mode)+self.title_list[self.mode])[:18]
    ...
    def receive(self, event, *args):
        if event == 'update_eta':
            print('SET ETA:', args[0], 'Mode:', self.mode)
            self.eta = args[0]
            self.render()
        elif event == 'set_state':
            print('SET TITLE STATE', args[0])
            self.mode = args[0]
            self.render()
...
class ControllerActions(Controller):
    ...
    def get_title(self):
        return super(ControllerActions, self).get_title()+str(self.mode)
...
def main():
    items = [
        ControllerTitle(), ControllerOnline(3), ControllerOffline(5), ControllerActions()
    ]
    core = Program()
